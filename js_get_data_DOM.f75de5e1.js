...
const averagePopulation = validCount === 0 ? 0 : Math.round(totalPopulation / validCount);
const totalElements = document.querySelector('.total-population');
if (totalElements) totalElements.textContent = totalPopulation.toLocaleString('en-US');
const averageElements = document.querySelector('.average-population');
if (averageElements) averageElements.textContent = Math.round(averagePopulation).toLocaleString('en-US');
